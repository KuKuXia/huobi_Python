...
class RestfulTestCaseSeq:
    ...
    def test_wallet(self):
        ...
        result, tc.server_req_cost, tc.server_api_cost = wallet_client.get_sub_user_deposit_address(sub_uid=g_sub_uid,
                                                                                                    currency="btc")
        tc.run_status = RunStatus.SUCCESS if result and len(result) else RunStatus.FAILED
        tc.add_record()
    ...
    def test_trade_create_cancel_orders(self):
        ...
        if create_result and len(create_result):
            ...
            # case batch_create_order
            tc = TimeCost(function_name=trade_client.batch_create_order.__name__)
            result, tc.server_req_cost, tc.server_api_cost = trade_client.cancel_orders(symbol=trade_symbol,
    ...
    def test_account(self):
        ...
        result, tc.server_req_cost, tc.server_api_cost = account_client.post_sub_uid_management(sub_uid=g_sub_uid,
        ...
        result, tc.server_req_cost, tc.server_api_cost = account_client.transfer_between_parent_and_subuser(
            sub_uid=g_sub_uid, currency="usdt", amount=10, transfer_type=TransferMasterType.OUT)
        result, tc.server_req_cost, tc.server_api_cost = account_client.transfer_between_parent_and_subuser(
            sub_uid=g_sub_uid, currency="usdt", amount=10, transfer_type=TransferMasterType.IN)
        tc.run_status = RunStatus.SUCCESS if result and result > 0 else RunStatus.FAILED
        tc.add_record()
